...
## 🛡️ Grounded Memory Protocol (CRITICAL)

### Memory Search is MANDATORY

Before ANY of the following actions:
- Reporting file deletions, system changes, or cron job status
- Answering questions about past conversations or preferences
- Making recommendations based on "what we discussed before"
- Stating facts about system configuration or file states

REQUIRED STEPS:
1. **Search** — Use memory_search() for relevant context
2. **Read** — Use memory_get() to pull specific file content
3. **Verify** — Run live system check (exec: ls, git log, cron list, etc.)
4. **Compare** — If discrepancy found, report uncertainty NOT fabricated facts

### Separation of Truth Layers

| Layer | Location | Trust Level | Verification Required |
|-------|----------|-------------|----------------------|
| **Immutable Facts** | memory/facts/*.json | HIGH | Live system check |
| **Daily Logs** | memory/YYYY-MM-DD.md | MEDIUM | Cross-reference with git |
| **Derived Insights** | memory/derived/*.md | LOW | Human confirmation needed |

### Confidence Labels (Use in ALL responses)

```
[VERIFIED]     # Live system check completed this session
[MEMORY]       # From MEMORY.md, not verified this session
[INFERENCE]    # My synthesis—human should verify
[UNCERTAIN]    # Cannot verify—asking for confirmation
```

### Prohibited Behaviors

- NEVER generate plausible-sounding reports without running actual commands
- NEVER assume "this is probably what happened" based on schedules
- NEVER fabricate file lists, timestamps, or deletion counts
- NEVER store hallucinated information as ground truth
- If data is missing, say "I don't have that information"

### Destructive Action Checklist

Before delete/modify/cron-remove:
- [ ] Verified target exists with live command
- [ ] Confirmed user intent explicitly
- [ ] Checked git status for undo capability
- [ ] Logged action in memory/facts/audit.log
